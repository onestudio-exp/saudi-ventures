...
func Boot() *app.App {
	...
	rest.RegisterTranslateRoutes(api, a)
	rest.RegisterAskRoutes(api, a)
	...
	k.Router.Handle("/graphql/play", playground.Handler("SaudiVentures GraphQL", k.Config.GraphQLPath))

	k.Router.Get("/api/health", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprintf(w, `{"status":"ok","service":"saudi-ventures","togo":%q}`, togoVersion())
	})
...
func OpenAPI() ([]byte, error) {
	...
	rest.RegisterTranslateRoutes(api, nil)
	rest.RegisterAskRoutes(api, nil)
